...
pair<string, string> divideStrings(string a, string b) {
    ...
    if (compareStrings(a, b) < 0) return make_pair("0", a);
...
string powerStrings(string base, string expStr) {
    ...
    string res = "1";
    while (exp > 0) {
        if (exp & 1ULL) res = multiplyStrings(res, base);
        exp >>= 1;
        if (exp) base = multiplyStrings(base, base);
    }
    return stripLeadingZeros(res);
}
    ...
    for (int i = 0; i < (int)expr.length(); i++) {
        ...
        if (c >= '0' && c <= '9') cur += c;
            ...
            while (oTop > 0 && precedence(op[oTop - 1]) >= precedence(c)) {
                ...
                else if (oper == 'x' || oper == 'X') res = multiplyStrings(a, b);
                else if (oper == '/') res = divideStrings(a, b).first;
                num[nTop++] = res;
            }
            op[oTop++] = c;
        }
